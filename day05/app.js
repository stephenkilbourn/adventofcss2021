const episodes = [
  ...
  {
    'id': 9,
    'name': 'The Tech Behind SelfTeach.me'
  },
  {
    'id': 10,
...
const renderEpisodes = () => {
  const episodesHTMLString = episodes.map( episode => {
    const isChecked = checkedBoxesMap[episode.id]
    const checkedString = isChecked ? 'checked' : ''
    return `
      <li>
      <label for="episode-${episode.id}">
        <input type="checkbox" ${checkedString} class="episode-${episode.id}" id="episode-${episode.id}" onClick="handleClick(event, ${episode.id})"/>
